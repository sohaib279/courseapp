...
function Purchases() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${BACKEND_URL}/user/purchases`, {
        headers: { Authorization: `Bearer ${token}` },
        withCredentials: true,
      })
      .then((res) => setPurchases(res.data.courseData))
  ...
  return (
    <div className="flex h-screen">
      <aside
        className={`fixed inset-y-0 left-0 w-64 bg-gray-100 p-5 transform transition-transform duration-300 ${
          isSidebarOpen ? "translate-x-0" : "-translate-x-full"
        } md:translate-x-0`}
      >
        <nav className="mt-16">
          <ul>
            ...
            <li><span className="flex items-center text-blue-500"><FaDownload className="mr-2" /> Purchases</span></li>
            <li>
              {token ? (
                <button onClick={handleLogout} className="flex items-center">
                  <IoLogOut className="mr-2" /> Logout
      ...
      <main className="flex-1 p-8 bg-gray-50 md:ml-64">
        ...
        {purchases.length ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {purchases.map(({ title, description, price, image }, index) => (
              <div key={index} className="bg-white rounded-lg shadow-md p-6">
                ...
                <div className="text-center mt-4">
                  ...
                  <span className="text-green-700 font-semibold">${price} only</span>
                </div>
              </div>
            ))}
          </div>
